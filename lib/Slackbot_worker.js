...
module.exports = function(botkit, config) {
    ...
    bot.sendWebhook = function(options, cb) {
        ...
        if(process.env['selfSigned']) {
            postOptions.ca = fs.readFileSync(process.env['selfSigned']);
        }
    ...
    bot.startRTM = function(cb) {
        bot.api.rtm.start({
            ...
            if(process.env['selfSigned']) {
                wsOptions.ca = fs.readFileSync(process.env['selfSigned']);
            }
            bot.rtm = new Ws(res.url, null, wsOptions);
            bot.msgcount = 1;
    ...
    bot.replyPublicDelayed = function(src, resp, cb) {
        if (!src.response_url) {
            ...
            if(process.env['selfSigned']) {
                requestOptions.ca = fs.readFileSync(process.env['selfSigned']);
            }
